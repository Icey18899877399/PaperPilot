...
class ChatAgent(BaseAgent):
    ...
    async def run(self, paper_id: str, question: str, trace_id: str) -> ChatResponse:
        ...
        videos = await self._recommend_videos(question, evidence.context)
        self._log_answer(trace_id, len(evidence.citations), len(videos))
        return ChatResponse(
            answer=answer,
    ...
    async def _recommend_videos(self, question: str, context: str) -> list:
        """Prefer public recommendations while supporting legacy/test catalogs."""
        recommend_public = getattr(self.videos, "recommend_public", None)
        if callable(recommend_public):
            return await recommend_public(question, context=context)
        return self.videos.recommend(question, context=context)
    ...
    def _log_answer(self, trace_id: str, citation_count: int, video_count: int) -> None:
        self.log(
            trace_id,
            "answer",
            f"检索{citation_count}个片段，推荐{video_count}个B站视频入口",
        )
